...
| FR-EMG-02 | partial | Two producers, one of which now exists. **Server-side:** `fastapi_app/services/threat_fusion.py` and `fastapi_app/routers/alerts.py` implement the decision path in full. All three signals now have producers as of 2026-09-02. `mobile/lib/features/safety/data/threat_pipeline.dart` arms with a Safe Journey and drives them: `mobile/lib/core/audio/threat_phrase_classifier.dart` scores recognised speech, `mobile/lib/features/devices/data/weapon_detection_service.dart` scores the glasses' video against `mobile/assets/models/weapon_yolov8n_fp16.tflite`, and `mobile/lib/features/safety/data/threat_signal_aggregator.dart` posts all three to the /alerts/analyze endpoint. Still partial for two reasons: no physical glasses have been paired, so the video path is verified only against a synthetic MJPEG stream, The camera is paired by network address in `mobile/lib/features/devices/presentation/widgets/glasses_pairing_sheet.dart`, which proves the address answers as a SafeHer camera before saving it. Microphone contention between threat listening and evidence capture is arbitrated by `mobile/lib/core/audio/microphone_arbiter.dart`, and evidence always wins. The two platforms score frames differently -- Android continuously on the phone, web by sampling to the /alerts/weapon-frame endpoint, which is a weaker guarantee and is published as such. **On the glove:** `glove/firmware/SafeHer_Glove_Final/SafeHer_Glove_Final.ino` (hardware-validated on a real ESP32-C3 on 2026-09-10) runs XGBoost on the ESP32 and notifies over BLE with no server in the path; `mobile/lib/features/devices/domain/glove_threat_detector.dart` votes on it and `mobile/lib/features/safety/data/glove_auto_trigger.dart` fires the countdown without needing a frame to be drawn. `mobile/lib/core/background/safety_foreground_service.dart` keeps that alive off screen. `mobile/lib/core/detection/detection_sources.dart` reports which of the two is actually live rather than letting either imply the other. Partial, not done: no physical glove has been paired, so this is verified only against a fake BLE service. |
...
| §6.1 three-model pipeline | partial | `fastapi_app/services/threat_models.py` — contract, registry and `POST /alerts/analyze` ready. **Weapon: trained and wired**, `mobile/assets/models/weapon_yolov8n_fp16.tflite`, mAP@0.5 0.906 measured on the exported artifact against 0.907 for the PyTorch original. **Motion:** the glove's XGBoost is trained and live. **Audio: trained and wired**, `mobile/assets/models/phrase_classifier.json` — platform speech recognition into a TF-IDF classifier, 0.905 accuracy at 0.968 recall on the elevated/normal boundary over held-out phrasings. The CNN+LSTM that previously sat here was removed: it scored 98.9% on Speech Commands and 54.5% on the phrases that matter, below a plain fuzzy string match. See `docs/WEAPON_INFERENCE_PLACEMENT.md` for why the weapon model runs on the phone and not the glasses |
