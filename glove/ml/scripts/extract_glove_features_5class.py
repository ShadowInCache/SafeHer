#!/usr/bin/env python3
"""Extract sliding-window features from SafeHer glove recordings, 5-class
scheme (PUSH/PULL/JERK merged into SUDDEN_MOVEMENT).

Copy of extract_glove_features.py with only LABELS and OUTPUT_PATH changed
to match the new class scheme and dataset/sudden_movement/ folder created
for it. The original 7-class script and its output are untouched - this is
a separate, parallel pipeline reading from the real, on-disk 5-class
dataset (dataset/normal, dataset/sudden_movement, dataset/shaking,
dataset/twisting, dataset/fall).
"""
...
OUTPUT_PATH = FEATURES_DIR / "glove_5class_features_from_raw.csv"
...
def print_summary(feature_df: pd.DataFrame, errors: list[str]):
    print("SAFEHER GLOVE FEATURE EXTRACTION SUMMARY (5-class)")
    print("-" * 50)
