#!/usr/bin/env python3
"""5-fold recording-level CV: does removing the 18 mislabeled FALL recordings
(fall_018, fall_020, fall_025-fall_040 - identified via
dataset_quality_audit/false_fall_temporal_analysis as having most of their
sliding-window features look like NORMAL/quiet motion despite being labeled
FALL) change the SAME V5 training configuration's cross-validated
performance? Development/CV only - no locked-test-set evaluation, no final
candidate trained, no decision made here about the frozen production model.

3 of those 18 deleted recordings (fall_018, fall_020, fall_025) were part of
V5's original 22-recording locked test set. This run EXCLUDES all 22 of that
split's recording IDs from CV regardless of whether they still exist on disk
(the remaining 19 continue to be protected from CV; the 3 deleted ones are
simply absent from both CV and the "locked test set" until re-recorded).

Configuration is copied EXACTLY from train_glove_7class_xgboost_v5.py's
XGBClassifier call - same as cv_v5_expanded_5fold.py's V5_XGB_PARAMS.
...
FEATURES_PATH = PROJECT_ROOT / "features" / "glove_7class_features_v4_fall_cleanup.csv"
V5_SPLIT_PATH = PROJECT_ROOT / "models" / "glove_7class_v5_train_test_split.json"
DATASET_ROOT = PROJECT_ROOT / "dataset"

OUT_DIR = PROJECT_ROOT / "models" / "glove_7class_v5_post_fall_cleanup_cv"
...
# (same values as cv_v5_expanded_5fold.py's V5_XGB_PARAMS). Deliberately does
...
def main():
    ...
    print(f"Of those, MISSING from disk (deleted as mislabeled FALL data, not yet re-recorded): {missing_locked}")
    assert missing_locked == ["fall_018", "fall_020", "fall_025"], (
        f"expected exactly the 3 known-deleted locked-test recordings missing, got {missing_locked}"
    )
    print("[OK] Missing set matches exactly the 3 known-deleted recordings - excluding all 22 IDs from CV regardless.")
    ...
    print(f"Development recordings (total - 19 present locked-test recordings): {n_dev_recordings}")
    ...
    # ============================ WRITE OUTPUTS ============================
    report = {
        "experiment": "v5_post_fall_cleanup_cv",
        "description": "5-fold recording-level StratifiedGroupKFold CV, exact V5 training config, on the dataset AFTER deleting the 18 mislabeled FALL recordings identified by dataset_quality_audit / false_fall_temporal_analysis. Locked test set excluded entirely (3 of its 22 members are among the deleted recordings and are pending re-recording).",
        "deleted_fall_recordings": [
            "fall_018", "fall_020", "fall_025", "fall_026", "fall_027", "fall_028", "fall_029", "fall_030",
            "fall_031", "fall_032", "fall_033", "fall_034", "fall_035", "fall_036", "fall_037", "fall_038",
            "fall_039", "fall_040",
        ],
    ...
    with open(OUT_DIR / "v5_post_fall_cleanup_cv_report.json", "w") as f:
        json.dump(report, f, indent=2, default=str)

    with open(OUT_DIR / "v5_post_fall_cleanup_cv_report.txt", "w") as f:
        f.write("SAFEHER V5-CONFIGURATION 5-FOLD CV AFTER DELETING 18 MISLABELED FALL RECORDINGS\n")
        f.write("=" * 70 + "\n\n")
        f.write("Locked test set: EXCLUDED, NEVER EVALUATED IN THIS EXPERIMENT.\n")
        f.write(f"3 of the 22 locked-test recordings (fall_018/020/025) are pending re-recording.\n")
        ...
        f.write("NOTE: 3 locked-test recordings (fall_018/020/025) still need to be re-recorded\n")
        f.write("before any final candidate can be evaluated on the complete locked test set.\n")
