#!/usr/bin/env python3
"""Train the 6-class SIZE-CONSTRAINED candidate (PUSH+PULL merged into
...
test set (same 22 recording_ids as always - push_003/push_006/pull_008/
pull_010 are now scored as SUDDEN_MOVEMENT). Compared against the ORIGINAL
frozen production V5 model (7-class) evaluated on the same set, with its
PUSH/PULL predictions collapsed to SUDDEN_MOVEMENT for a fair comparison.

Does NOT modify, retrain, or overwrite the original V5 model file - loaded
fresh, read-only, purely for comparison.
"""
...
FEATURES_PATH = PROJECT_ROOT / "features" / "glove_6class_features_v1_merged_pushpull.csv"
...
OUT_DIR = PROJECT_ROOT / "models" / "glove_6class_v6_merged_pushpull_final"
OUT_DIR.mkdir(parents=True, exist_ok=True)

CANDIDATE_MODEL_PATH = OUT_DIR / "safeher_glove_6class_v6_merged_pushpull_candidate_xgboost.json"
TRAINING_REPORT_PATH = OUT_DIR / "v6_merged_pushpull_candidate_training_report.json"
COMPARISON_TXT_PATH = OUT_DIR / "v6_merged_pushpull_candidate_vs_original_v5_comparison.txt"
COMPARISON_JSON_PATH = OUT_DIR / "v6_merged_pushpull_candidate_vs_original_v5_comparison.json"

LABELS_6 = ["NORMAL", "JERK", "SUDDEN_MOVEMENT", "SHAKING", "TWISTING", "FALL"]
LABEL_TO_ID_6 = {l: i for i, l in enumerate(LABELS_6)}
...
SIZE_CONSTRAINED_XGB_PARAMS = dict(
    ...
    random_state=42, objective="multi:softprob", num_class=len(LABELS_6),
...
def main():
    ...
    y_train = devset["label"].map(LABEL_TO_ID_6).astype(int)

    print("\nTraining 6-class size-constrained candidate on ALL 208 dev recordings...")
    ...
    training_report = {
        "experiment": "v6_merged_pushpull_candidate_final",
        "description": "6-class candidate: PUSH+PULL merged into SUDDEN_MOVEMENT. Config A hyperparameters (V5 config + min_child_weight=30) for ESP32-C3 flash fit.",
        "class_order": LABELS_6,
    ...
    y_test_6 = testset["label"].map(LABEL_TO_ID_6).astype(int).to_numpy()
    ...
    cand_pred_labels = np.array([LABELS_6[i] for i in cand_pred_id])
    ...
    # Collapse original's PUSH/PULL predictions to SUDDEN_MOVEMENT for fair comparison
    orig_pred_labels6 = np.array(["SUDDEN_MOVEMENT" if l in ("PUSH", "PULL") else l for l in orig_pred_labels7])
    ...
    def eval_6class(pred_labels, true_labels):
        labels_range = LABELS_6
    ...
    candidate_eval = eval_6class(cand_pred_labels, y_test_true_labels)
    original_eval_collapsed = eval_6class(orig_pred_labels6, y_test_true_labels)
    ...
    comparison = {
        ...
        "note": "original_v5 evaluated as its native 7-class model, then PUSH/PULL predictions collapsed to SUDDEN_MOVEMENT for a fair comparison against the 6-class candidate.",
    ...
    with open(COMPARISON_TXT_PATH, "w") as f:
        f.write("V6 MERGED PUSH/PULL CANDIDATE vs ORIGINAL V5 (collapsed) - LOCKED TEST (22 recordings)\n")
        f.write("=" * 70 + "\n\n")
