#!/usr/bin/env python3
"""Train ONE final v5-expanded candidate on all 198 development recordings
(220 total - V5's own 22 locked test recordings), using the EXACT V5 training
configuration (read from train_glove_7class_xgboost_v5.py). Then perform
EXACTLY ONE official evaluation on the locked 22-recording test set, and
compare against the ORIGINAL production V5 model evaluated on that same set.
...
CANDIDATE_MODEL_PATH = OUT_DIR / "safeher_glove_7class_v5_expanded_candidate_xgboost.json"
TRAINING_REPORT_PATH = OUT_DIR / "v5_expanded_candidate_training_report.json"
LOCKED_TEST_EVAL_PATH = OUT_DIR / "v5_expanded_candidate_locked_test_evaluation.json"
COMPARISON_PATH = OUT_DIR / "v5_expanded_candidate_vs_original_v5_comparison.json"
COMPARISON_TXT_PATH = OUT_DIR / "v5_expanded_candidate_vs_original_v5_comparison.txt"
...
# (same values used in cv_v5_expanded_5fold.py's per-fold training).
V5_XGB_PARAMS = dict(
    n_estimators=600,
    max_depth=6,
    learning_rate=0.05,
...
def main():
    # ---- identify locked test set (V5's own 22, per prior confirmation) ----
    ...
    assert n_dev_recordings == 198, f"expected 198 dev recordings, got {n_dev_recordings}"
    ...
    # ================= TRAIN ONE CANDIDATE ON ALL 198 DEV RECORDINGS =================
    X_train = devset[feature_cols].astype(float)
    y_train = devset["label"].map(LABEL_TO_ID).astype(int)

    print("\nTraining candidate on ALL 198 development recordings, exact V5 config...")
    ...
    print("No eval_set, no early stopping, no validation split - matches train_glove_7class_xgboost_v5.py exactly "
          "(that script also fits on its full training partition with no held-out validation).")
    ...
    training_report = {
        "experiment": "v5_expanded_candidate_final",
    ...
    # confirm original file still untouched after evaluation too
    ...
    comparison = {
        ...
        "original_v5": {
            ...
            "push_recall": original_eval["per_class"]["PUSH"]["recall"],
            "pull_recall": original_eval["per_class"]["PULL"]["recall"],
        ...
        "candidate": {
            ...
            "push_recall": candidate_eval["per_class"]["PUSH"]["recall"],
            "pull_recall": candidate_eval["per_class"]["PULL"]["recall"],
        ...
        "delta_candidate_minus_original": {
            ...
            "push_recall": candidate_eval["per_class"]["PUSH"]["recall"] - original_eval["per_class"]["PUSH"]["recall"],
            "pull_recall": candidate_eval["per_class"]["PULL"]["recall"] - original_eval["per_class"]["PULL"]["recall"],
        },
        "per_class_both": {
    ...
    with open(COMPARISON_TXT_PATH, "w") as f:
        f.write("V5 EXPANDED CANDIDATE vs ORIGINAL V5 - LOCKED TEST EVALUATION (22 recordings)\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"{'PUSH recall':<20} {comparison['original_v5']['push_recall']:>15.4f} {comparison['candidate']['push_recall']:>15.4f} {comparison['delta_candidate_minus_original']['push_recall']:>+10.4f}\n")
        f.write(f"{'PULL recall':<20} {comparison['original_v5']['pull_recall']:>15.4f} {comparison['candidate']['pull_recall']:>15.4f} {comparison['delta_candidate_minus_original']['pull_recall']:>+10.4f}\n")
