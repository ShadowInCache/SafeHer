#!/usr/bin/env python3
"""5-fold recording-level CV: after recollecting 16 new FALL recordings
(fall_025-fall_040, with a tighter capture protocol - 4s duration, motion
performed within ~1s of recording start) to replace 18 that were deleted for
being mislabeled (dataset_quality_audit/false_fall_temporal_analysis found
most of their sliding-window features looked like NORMAL/quiet motion
despite being labeled FALL).

fall_018 and fall_020 are still missing on disk - the collection tool's
auto-numbering picks the next number after the highest existing file rather
than backfilling gaps, so those two slots were skipped. fall_025 (also
originally a locked-test-set member) DID get recreated as part of the normal
numbering sequence. This run therefore expects exactly 2 missing locked-test
recordings, not 3.
...
XGBClassifier call - identical V5_XGB_PARAMS to the prior CV scripts in this
series (cv_v5_expanded_5fold.py, cv_v5_post_fall_cleanup.py).
...
FEATURES_PATH = PROJECT_ROOT / "features" / "glove_7class_features_v5_fall_recollected.csv"
V5_SPLIT_PATH = PROJECT_ROOT / "models" / "glove_7class_v5_train_test_split.json"
DATASET_ROOT = PROJECT_ROOT / "dataset"

OUT_DIR = PROJECT_ROOT / "models" / "glove_7class_v5_fall_recollected_cv"
...
def main():
    ...
    assert missing_locked == ["fall_018", "fall_020"], (
        f"expected exactly fall_018/fall_020 missing (fall_025 was recreated), got {missing_locked}"
    )
    ...
    # ============================ WRITE OUTPUTS ============================
    report = {
        "experiment": "v5_fall_recollected_cv",
        "description": "5-fold recording-level StratifiedGroupKFold CV, exact V5 training config, after recollecting 16 new FALL recordings (fall_025-fall_040) with a tighter capture protocol to replace 18 deleted mislabeled ones. fall_018/fall_020 still missing (numbering gap, not backfilled).",
        "recollected_fall_recordings": [f"fall_0{i}" for i in range(25, 41)],
    ...
    with open(OUT_DIR / "v5_fall_recollected_cv_report.json", "w") as f:
        json.dump(report, f, indent=2, default=str)

    with open(OUT_DIR / "v5_fall_recollected_cv_report.txt", "w") as f:
        f.write("SAFEHER V5-CONFIGURATION 5-FOLD CV AFTER RECOLLECTING FALL DATA\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write("\nCOMPARISON (20/20 CV / 30/30 CV / post-cleanup-20-fall-only / this run):\n")
        f.write(f"Accuracy:    {previous_20_20_cv['accuracy']['mean']:.4f} / {previous_30_30_cv['accuracy']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['accuracy']['mean']:.4f} / {summary['accuracy']['mean']:.4f}\n")
        f.write(f"Macro F1:    {previous_20_20_cv['macro_f1']['mean']:.4f} / {previous_30_30_cv['macro_f1']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['macro_f1']['mean']:.4f} / {summary['macro_f1']['mean']:.4f}\n")
        f.write(f"FALL recall: {previous_20_20_cv['fall_recall']['mean']:.4f} / {previous_30_30_cv['fall_recall']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['fall_recall']['mean']:.4f} / {summary['FALL_recall']['mean']:.4f}\n")
