#!/usr/bin/env python3
"""5-fold recording-level CV: the locked test set is now FULLY COMPLETE
(all 22 recordings present on disk) for the first time since the FALL
data-quality investigation began. fall_018 and fall_020 - previously stuck
as numbering gaps that the collection tool never backfilled - were recorded
fresh (multi-burst 10s protocol, spot-checked for burst spread) and then
renamed into place, which only changes the filename/recording_id, not the
underlying sensor data.

This is still development/CV only - no locked-test-set evaluation, no final
candidate trained, no decision made here about the frozen production model.
It exists to get one more honest number with the dataset in its complete
state before deciding whether to train a real final candidate and spend the
...
OUT_DIR = PROJECT_ROOT / "models" / "glove_7class_v5_fall_complete_cv"
...
def main():
    ...
    assert missing_locked == [], (
        f"expected the locked test set to be fully complete (0 missing), got {missing_locked}"
    )
    print("[OK] All 22 locked test recordings present on disk - the locked test set is COMPLETE.")
    ...
    previous_single_burst_4s_cv = {
        ...
        "note": "first recollection attempt: 4s duration, single motion burst near start - same structural flaw as originally deleted data. Deleted and redone.",
    }
    previous_multiburst_2_missing_cv = {
        "accuracy": {"mean": 0.7573}, "macro_f1": {"mean": 0.7407},
        "fall_recall": {"mean": 0.7338},
        "total_fall_to_normal": 90, "total_normal_to_fall": 63,
        "note": "corrected multi-burst-spread protocol (10s, 4-6 motions spread across the clip) - but fall_018/fall_020 were still missing (numbering gap) at the time of this run, so only 35 of 40 fall recordings were in the dev set.",
    }

    # ============================ WRITE OUTPUTS ============================
    report = {
        "experiment": "v5_fall_complete_cv",
        "description": "5-fold recording-level StratifiedGroupKFold CV, exact V5 training config, with the FULL 40-recording FALL dataset and the complete, gap-free 22-recording locked test set for the first time. fall_018/fall_020 were recorded with the corrected multi-burst protocol and renamed into their numbering-gap slots (rename only changes recording_id, not sensor data).",
        "locked_test_recordings_all_22": sorted(locked_test_recordings),
        "locked_test_set_complete": True,
        ...
        "comparison_previous_multiburst_2_missing_cv": previous_multiburst_2_missing_cv,
        "locked_test_set_used": False,
        "final_candidate_trained": False,
    }
    with open(OUT_DIR / "v5_fall_complete_cv_report.json", "w") as f:
        json.dump(report, f, indent=2, default=str)

    with open(OUT_DIR / "v5_fall_complete_cv_report.txt", "w") as f:
        f.write("SAFEHER V5-CONFIGURATION 5-FOLD CV, COMPLETE FALL DATASET (40/40, LOCKED TEST SET 22/22)\n")
        f.write("=" * 70 + "\n\n")
        f.write("Locked test set: EXCLUDED, NEVER EVALUATED IN THIS EXPERIMENT.\n")
        f.write("All 22 locked test recordings present on disk - the locked test set is COMPLETE.\n")
        ...
        f.write("\nCOMPARISON (20/20 / 30/30 / post-cleanup-20-fall / single-burst-4s / multiburst-2-missing / this run):\n")
        f.write(f"Accuracy:    {previous_20_20_cv['accuracy']['mean']:.4f} / {previous_30_30_cv['accuracy']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['accuracy']['mean']:.4f} / {previous_single_burst_4s_cv['accuracy']['mean']:.4f} / {previous_multiburst_2_missing_cv['accuracy']['mean']:.4f} / {summary['accuracy']['mean']:.4f}\n")
        f.write(f"Macro F1:    {previous_20_20_cv['macro_f1']['mean']:.4f} / {previous_30_30_cv['macro_f1']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['macro_f1']['mean']:.4f} / {previous_single_burst_4s_cv['macro_f1']['mean']:.4f} / {previous_multiburst_2_missing_cv['macro_f1']['mean']:.4f} / {summary['macro_f1']['mean']:.4f}\n")
        f.write(f"FALL recall: {previous_20_20_cv['fall_recall']['mean']:.4f} / {previous_30_30_cv['fall_recall']['mean']:.4f} / {previous_post_cleanup_cv_20_fall_only['fall_recall']['mean']:.4f} / {previous_single_burst_4s_cv['fall_recall']['mean']:.4f} / {previous_multiburst_2_missing_cv['fall_recall']['mean']:.4f} / {summary['FALL_recall']['mean']:.4f}\n")
        f.write(f"NORMAL->FALL: -- / -- / {previous_post_cleanup_cv_20_fall_only['total_normal_to_fall']} / {previous_single_burst_4s_cv['total_normal_to_fall']} / {previous_multiburst_2_missing_cv['total_normal_to_fall']} / {total_normal_to_fall}\n")

        f.write("\nlocked_test_set_used: False\n")
        f.write("final_candidate_trained: False\n")
        f.write("Dataset and locked test set are now both complete - ready for a final candidate\n")
        f.write("to be trained and evaluated exactly once on the locked test set, if desired.\n")
